...
sub test_requires_network() {
    ...
    unless ( has_network ( $host ) ) {
        plan skip_all =>
            "Test requires network($host) which is not available now.";
    }
}
...
=item test_requires_root

This function skip all tests if the user is not root

=cut

sub test_requires_root {
    if ($> != 0){
        plan skip_all => "Test requires to be run as root";
    }
}
...
sub test_plugin_deps() {
    ...
    my $meta = eval { YAML::LoadFile( $file ) }
        or die "reading $file failed:\n$@";
...
sub interpolate {
    ...
    # (but not \$foo::bar)
    $stuff
        =~ s/(?<!\\)                         # check there's no backslash before this
...
# use Data::Dumper;
# warn Dumper($config);
    Assurer->bootstrap( { config => $config } );
}
